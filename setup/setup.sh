#!/bin/bash
########################################
# setup.sh - Veil Setup Script
########################################

if [ `whoami` == 'root' ]
then
  # Install Architecture Dependent Dependencies With Root
  if [ `uname -m` == 'x86_64' ]
  then
  ...
  # Install Non-Architecture Dependent Dependencies With Root
  apt-get install mingw-w64 monodoc-browser monodevelop mono-mcs wine python python-crypto
elif [ `which sudo|wc -l` == '1' ]
then
  # Install Architecture Dependent Dependencies With Sudo
  if [ `uname -m` == 'x86_64' ]
  then
